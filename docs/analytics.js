/**
 * analytics.js - FINAL (integrates all fixes, preserves original UX)
 *
 * Fixes applied:
 * - token-list.json fallback for symbols/decimals/logo
 * - safe read provider (prefer RPC) to avoid injected provider RPC failures
 * - safe oracle reads with fallbacks
 * - batch processing of pairs to avoid rpc overload
 * - precise LP share calculation using BigInt and subtracting MINIMUM_LIQUIDITY (minted to address(0))
 * - restored top cards updates (globalTVL, userTotalLiquidity, synthetic metrics)
 * - session restore guard: only auto-reconnect if SessionManager.isActive()
 * - no automatic connect on load unless session active (keeps UX aligned with swap/markets)
 * - explorer link generation fixed (no trailing slash)
 *
 * Keep only required changes; rest of original logic & classes preserved.
 */
...
// MINIMUM_LIQUIDITY constant from VibePair (minted to address(0) on first mint)
const PAIR_MINIMUM_LIQUIDITY = 100000n; // 1e5

// Token list fallback map
let TOKEN_MAP = {};
  ...
  try {
    ...
    json.tokens.forEach(t => {
      try {
        ...
        if (!cid || !addr) return;
        TOKEN_MAP[`${cid}:${addr}`] = t;
      } catch (e) { /* ignore malformed */ }
    });
    console.log("Token list loaded:", Object.keys(TOKEN_MAP).length, "entries");
  } catch (e) {
    console.warn("Could not load token-list.json:", e);
  }
}

/**
 * Read token metadata (tries on-chain, decodes bytes32 symbols, falls back to token-list)
 * Returns: { symbol, decimals, meta }
 */
async function readTokenMeta(tokenAddr, readProvider, chainId) {
  if (!tokenAddr) return { symbol: "UNK", decimals: 18, meta: null };
  const key = `${Number(chainId)}:${tokenAddr.toLowerCase()}`;
  const meta = TOKEN_MAP[key] || null;
  let symbol = "";
  let decimals = 18;

  if (readProvider) {
    try {
      const t = new ethers.Contract(tokenAddr, ANALYTICS_ERC20_ABI, readProvider);
      try { const s = await t.symbol().catch(() => ""); if (s) symbol = s; } catch (e) {}
      try { const d = await t.decimals().catch(() => 18); decimals = Number(d || 18); } catch (e) { decimals = 18; }
    } catch (e) {}
  }

  // decode bytes32-like symbol if returned
  try {
    if (typeof symbol === "string" && symbol.startsWith("0x")) {
      try {
        const decoded = ethers.toUtf8String(symbol).replace(/\u0000/g, '');
        if (decoded && decoded.trim().length) symbol = decoded;
      } catch (e) {}
    }
  } catch (e) {}

  if ((!symbol || symbol === "") && meta) {
    symbol = meta.symbol || meta.name || "";
    if (meta.decimals) decimals = Number(meta.decimals);
  }

  if (!symbol || symbol === "") symbol = "UNK";
  return { symbol, decimals: Number(decimals || 18), meta };
}
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // Guarded auto-reconnect: only if session was active previously
    if (window.checkAutoConnect && window.SessionManager && typeof window.SessionManager.isActive === "function") {
      try {
  ...
  try {
    ...
    if (ACTIVE) {
      if (window.localStorage) {
        try { localStorage.setItem("VIBE_SESSION_ACTIVE", "true"); } catch (e) {}
      }
...
function bindUIActions() {
  ...
  // Close account dropdown on outside click, and wallet modal overlay clicks
  window.onclick = (e) => {
    const modal = getEl('walletModal');
...
async function loadAnalytics() {
  ...
  try {
    ...
    // Resolve oracle address if available
    let oracle = null;
    ...
    for (let i = 0; i < pairAddresses.length; i += BATCH) {
      ...
      const promises = batch.map(async (pairAddr) => {
        try {
          ...
          const t0Meta = await readTokenMeta(t0Addr, readProvider, ACTIVE.chainId);
          const t1Meta = await readTokenMeta(t1Addr, readProvider, ACTIVE.chainId);

          const sym0 = t0Meta.symbol || "UNK";
          const sym1 = t1Meta.symbol || "UNK";
          const dec0 = Number(t0Meta.decimals || 18);
          const dec1 = Number(t1Meta.decimals || 18);
          ...
          // Calculate user's position precisely, using BigInt and subtracting minimum liquidity
          let myPositionUSD = 0;
          let myShare = 0;
          if (userAddress) {
            try {
              ...
              if (userBalRaw && totalSupplyRaw && totalSupplyRaw > 0n) {
                try {
                  const ub = BigInt(userBalRaw.toString());
                  const ts = BigInt(totalSupplyRaw.toString());
                  // Adjust denominator by removing MINIMUM_LIQUIDITY if present
                  const tsAdj = ts > PAIR_MINIMUM_LIQUIDITY ? (ts - PAIR_MINIMUM_LIQUIDITY) : ts;
                  if (tsAdj > 0n) {
                    const shareBp = (ub * 10000n) / tsAdj; // basis points 0..10000
                    const shareFraction = Number(shareBp) / 10000; // <= 1.0
                    if (!isNaN(shareFraction) && shareFraction > 0) {
                      myShare = shareFraction * 100;
                      myPositionUSD = pairTVL * shareFraction;
                      userTotalLiquidity += myPositionUSD;
                    }
                  } else {
                    // fallback: use formatUnits if adjusted denominator is zero
                    let lpDecimals = 18;
                    try {
                      if (typeof pair.decimals === 'function') {
                        const lpDecRaw = await pair.decimals().catch(() => 18);
                        lpDecimals = lpDecRaw ? Number(lpDecRaw) : 18;
                      }
                    } catch (e) { lpDecimals = 18; }
                    const userBal = parseFloat(ethers.formatUnits(userBalRaw, lpDecimals));
                    const totalSupply = parseFloat(ethers.formatUnits(totalSupplyRaw, lpDecimals));
                    if (!isNaN(userBal) && !isNaN(totalSupply) && totalSupply > 0) {
                  ...
                  try {
                    ...
                    myShare = 0; myPositionUSD = 0;
                  }
                }
              }
            } catch (errPos) {
              console.warn("Position calc error for pair", pairAddr, errPos);
            }
          }
...
function finalizeAnalytics(pairsData, globalTVL, oracle, userTotalLiquidity = undefined, synthetic = undefined) {
  ...
  // user's total liquidity card (if present)
